...
{
	while (readl(port->membase + DATA_TX_FREE) == 0)
		cpu_relax();
	...
	if (pending & IRQ_RX) {
		...
		while ((left = readl_relaxed(dcuart->port.membase + DATA_RX_COUNT))) {
			...
			while (left) {
				 ...
				 * these instances.
				 */
				char c = readl_relaxed(dcuart->port.membase + DATA_RX8) >> 8;
...
static u_int dcuart_tx_empty(struct uart_port *port)
{
...
{
	return (port->type == PORT_APPLE_DOCKCHANNEL) ? "Apple DockChannel UART" : NULL;
}
...
static void dcuart_set_termios(struct uart_port *port, struct ktermios *new, const struct ktermios *old)
{
}
...
static int dcuart_probe(struct platform_device *pdev)
{
	...
	struct resource *data_res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "data");
	if (IS_ERR(data_res))
		return PTR_ERR(data_res);
	...
	ret = devm_request_irq(&pdev->dev, dcuart->port.irq, dcuart_interrupt, IRQF_SHARED, dev_name(&pdev->dev), dcuart);

	disable_irq(dcuart->port.irq);
